...
| DECISION-074 | Reskin foundation — green-led/obsidian palette remap + Bricolage/Hanken fonts | tokens.css values remapped to prototype (names unchanged so components inherit); added `--color-teal`; fonts → Bricolage Grotesque + Hanken Grotesk; full plan in docs/RESKIN-PLAN.md | Decided |
| DECISION-075 | Reskin Phase 1 — app shell TopNav → persistent left sidebar + mobile bottom-nav/FAB | AppSidebar (desktop) + AppMobileNav (mobile FAB → /trips?modal=log); realises DECISION-019; TopNav/MobileNav retired in cleanup | Decided |
| DECISION-076 | Reskin Phase 2 — dashboard recompose: rolling-window timeline + bento | RollingWindowTimeline (verdict derives from getRiskStatus; watch lines 120/150), AbsenceHeatmap, PeakTrajectoryChart (+ getRollingWindowSeries), inline PlanTripSimulator; QuotaRing retired from dashboard | Decided |
| DECISION-077 | Reskin Phase 3 — Trips list recompose + List/Timeline toggle | List with conditional badges (abroad/planned/Crown), neutral Crown chip "doesn't count", peak-window span, single header stat, NEW Gantt timeline; bulk delete retained; sort/window-at-departure dropped | Decided |
| DECISION-078 | Reskin Phase 4 — Reports evidence pack (period selector + A4 live preview) | Two-column evidence pack; live preview mirrors the @react-pdf doc; presets map to ilr/custom; rolling history as secondary export; Pro-gating + engine unchanged; no previous-reports list | Decided |
| DECISION-079 | Reskin Phase 5 — Settings six jump-nav sections | Visa&ILR/Account/Subscription/Notifications/Appearance/Data&privacy with scrollspy; all four plans + Stripe checkout; notifications 120/150 copy on real 5 columns; theme chooser; actions/columns unchanged | Decided |
| DECISION-080 | Reskin Phase 6 — Marketing landing reskin | Hero headline + RollingWindowTimeline (SAFE sample, 120/150 watch lines) replaces ring mockup; new Comparison + HowItWorks; Features/Pricing kept (copy already correct: 10 trips, 4 plans) | Decided |
| DECISION-081 | Reskin Phase 7 — Onboarding restyled to new design system | layout/welcome/VisaForm/TripForm moved from hardcoded light-only hex to semantic tokens (now themes light+dark); flow, server actions, quota guard, overlap detection, copy, and all PostHog events unchanged | Decided |
| DECISION-082 | Reskin Phase 8 — Trip modal recomposed to single-sheet + success state | TripFlowClient 3-step wizard → one scrolling form (validate on submit); success screen (View in Trips/Log another/Done) replaces save-then-navigate; notes→"Reason for travel"; planned shown as derived read-only chip; all actions/what-if/overlap/PostHog events preserved | Decided |
| DECISION-083 | Reskin Phase 9 — Cleanup: retire dead code, rename font vars, prune tokens | Deleted QuotaRing/TopNav/MobileNav/Sidebar/TripsClient (no importers); renamed --font-manrope→--font-heading & --font-inter→--font-body across ~44 files; pruned dead --color-track/--shadow-ring-card; auth/error screens left out of scope | Decided |
| DECISION-084 | DateRangePicker — full-cell tap targets + month/year jump picker | Day hit area now fills the full grid column at 44px (was a 36px centred circle); centre label opens a year-stepper + 3×4 month grid so back-dated trips are ~3 taps; selection logic/contract unchanged; E2E calendar helpers repointed | Decided |
| DECISION-085 | Auth + error screens migrated to semantic tokens | error/not-found + 4 auth screens moved off hardcoded hex (now theme light+dark); Google logo fills + white-on-green wordmark kept; completes the deferral noted in DECISION-083 | Decided |
| DECISION-086 | Exclude docs/markdown from Tailwind's content scan | `@source not` for docs + *.md in globals.css; a className snippet quoted in an ADR was compiled to invalid `font-family: var(...)` and broke the build; markdown never supplies utilities | Decided |
| DECISION-087 | Remove redundant "Plan a trip" button from Trips page | Planning lives in the dashboard what-if simulator (DECISION-076); Trips header keeps only "Log trip"; modal plan mode still reachable via ?modal=plan (E2E) so no behaviour/test change | Decided |
| DECISION-088 | RollingWindowTimeline recomposed for hierarchy + non-colliding axis | One hero number (verdict → calm status pill); tidy baseline-aligned number/unit; grouped right-column spacing; month axis 6→4 ticks (fixes JulSeptNov… collision); items-start. Presentational only; marketing Hero inherits it | Decided |
| DECISION-089 | Speedometer gauge replaces the rolling-window timeline (signature graphic) | New WindowSpeedometer (270° conic-gradient gauge + needle, themed via tokens/color-mix) on dashboard + marketing Hero; RollingWindowTimeline deleted; colour/word still derive from getRiskStatus; spec adapted (themed not dark-only, kept JetBrains Mono not Space Grotesk, adaptive 2nd stat) | Decided |
| DECISION-090 | Speedometer fixes — visible dial + container-query layout | color-mix-in-conic-gradient didn't render (dial invisible) → explicit --gauge-safe/-caution/-breach rgba tokens; layout now @container-driven (wide card→3 columns dashboard, narrow card→stacked marketing Hero); tile made more compact; verified light+dark on :3001 | Decided |
| DECISION-091 | Speedometer dial moved to a CSS class (.gauge-dial) | Production JS minifier stripped `deg` before each var() in the inline conic-gradient string ("225deg, var(" → "225var("), invalidating it → dial invisible in prod only. Moved the fixed-geometry gradient to globals.css; verified on a next build + next start (computed style resolves correctly) | Decided |
