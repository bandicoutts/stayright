# StayRight — User Journeys (per-journey detail)

> Navigation throughout the logged-in app is the single top navigation bar [`TopNav`]: logo → Dashboard; primary links Dashboard · Trips · Reports · Settings; right side a light/dark theme toggle [`ThemeToggle`] and a user menu (avatar + name, green **PRO** badge for Pro users) whose popover holds Settings · Sign out. On mobile these links collapse into a hamburger drawer built into the same `TopNav`.
...
2. **When are you travelling?** — an inline calendar [`DateRangePicker`] on the left; a **live compliance panel** on the right. As soon as both dates are valid, the panel shows: *"This trip — N days,"* *"Days remaining after — N days,"* a progress bar (*"N / 180 days used"*), and a risk verdict (see states below). A breach shows a red warning: *"This trip would push you to X/180 days… You would breach the absence limit."* → **Next →**
...
**Exit:** Back to dashboard/trips — either dismissed via "Just checking," or with the new trip saved and the dashboard updated.
...
**Entry points:** Automatic immediately after first signup (email or Google); also re-entered any time a logged-in, un-onboarded user hits an app route.
...
**Entry points:** Reports nav → `/reports` [`ReportsClient`]. *"Reports & Exports — Generate PDF documents suitable for inclusion in an ILR application."*
...
**States:** Pro gate is enforced both in UI and server-side; empty (*"No absence data to report yet. Log your trips first…"*); loading (*"Generating…"*); client-side date-validation errors.

**Exit:** PDF downloads; the user stays on the Reports page. Free users branch into the paywall (P4).
...
**Entry points:** User menu → Settings → **Visa Details** tab [`SettingsClient`].
...
**States:** default / saving / *"Profile saved."* confirmation / field-validation error.

**Exit:** Stays on the Visa Details tab with the saved values; dashboard recalculates against the new profile.
...
**Entry points:** Settings → **Account** tab.
...
- **Help & Support** — links to `help@stayright.app` and the Help Centre (`/help` — *[out of scope for design review — placeholder page]*).
- **Danger Zone → Delete account** — reveals a confirmation requiring the user to type **"delete my account"** exactly; deletes everything and redirects to `/login`.

**States:** per sub-flow — default / saving / success confirmation / error; delete is gated until the confirmation phrase matches exactly.

**Exit:** Stays on the Account tab after most actions; **Manage subscription** hands off to the Stripe Portal (S5); **Delete account** ends at `/login`.
...
**Entry points:** Settings → **Notifications** tab.
...
**States:** Free (locked toggles + upsell box) / Pro default / saving / *"Preferences saved."* confirmation.

**Exit:** Stays on the Notifications tab; Free users branch into the paywall (P4) from the upsell box.
...
**Entry points:** Settings → Account → **"Manage subscription."**

**Path:** Opens the Stripe Billing Portal. The user can update payment method, change billing cadence, or cancel. Cancelling reverts to Free but keeps the user's data. Lifetime users have nothing to manage.

**States:** loading (opening portal) / portal session / returned to app.

**Exit:** Returns to `/settings` with the subscription state refreshed.
...
**Entry points:** `/` (landing) [`Hero`, `Features`, `Pricing`, `TrustBar`, `Footer`].
...
The footer also links to `/about`, `/blog`, `/contact` and `/help` — *[out of scope for design review — placeholder pages]*.

**States:** first visit (cookie banner present) / returning visit (banner dismissed).
...
**Entry points:** Any CTA, the nav *"Sign in,"* or a redirect from a protected route.
...
**States:** default / submitting / validation (passwords match, min 8 chars) / auth error (*"Incorrect email or password."*, *"Something went wrong with your login link"*) / verification-sent and reset-sent confirmation screens.

**Exit:** New users into onboarding (P5); returning users into the dashboard (P2).

---

## S8 — Theme & device adaptation (ambient)

**Goal:** Use the app comfortably in light or dark mode and on any screen size.

**Entry points:** Present on every screen.

**Path:** A **light/dark theme toggle** [`ThemeToggle`] sits in the top nav. On mobile the top-nav links collapse into a hamburger drawer, and the trip drawer becomes a bottom sheet.

**States:** light theme / dark theme; desktop nav / mobile hamburger drawer.

**Exit:** Ambient — no destination; the chosen theme persists across the session.
